riot.tag2('rts-model-wald-test-decision', '<h4>Supremum Wald test conclusion</h4> <table border="1" cellpadding="0" cellspacing="0" width="100%"> <tr> <td>Unit</td> <td>Formal intervention</td> <td>Estimated change point</td> <td>Intervention lag</td> <td>Supremum Wald-test decision</td> </tr> <tr each="{model, index in opts.models}"> <td>{model.unit_name}</td> <td>{theoretical_change_point(model)}</td> <td>{estimated_change_point(model)}</td> <td>{diff_change_point(model)}</td> <td>{supremum_wald_decision(model)} (p-val: {supremum_wald_p_value(model)}) </td> </tr> </table>', '', '', function(opts) {
        ...
        const self = this;
        self.theoretical_change_point = (model) => moment(model.dates[model.estimations.likelihood.best_index]).format(
            "MMM DD, YYYY");
        self.estimated_change_point = (model) => moment(model.dates[model.change_point.theoretical]).format(
            "MMM DD, YYYY");
        self.diff_change_point = (model) => moment(model.dates[model.estimations.likelihood.best_index]).from(
...
riot.tag2('rts-model-parameter-changes-table', '<h4>Unit-specific changes in level and slope</h4> <table border="1" cellpadding="0" cellspacing="0" class="double-header" width="100%"> <tr> <td rowspan="2">Unit</td> <td colspan="3">Changes in level</td> <td colspan="3">Changes in slope</td> </tr> <tr> <td>Estimate</td> <td>95% CI</td> <td>p-val</td> <td>Estimate</td> <td>95% CI</td> <td>p-val</td> </tr> <tr each="{model, index in opts.models}"> <td>{model.unit_name}</td> <td>{estimate_1(model)}</td> <td>{ci_1(model)}</td> <td>{pval_1(model)}</td> <td>{estimate_2(model)}</td> <td>{ci_2(model)}</td> <td>{pval_2(model)}</td> </tr> </table>', '', '', function(opts) {
