...
var annotations = function(opt_options) {
  ...
  this.defaultAnnotationProperties_ = jQuery.extend({
    'text': 'Description'
...
annotations.prototype.detachLabels = function() {
  for (var i = 0; i < this.annotations_.length; i++) {
    ...
    jQuery(a.lineDiv).remove();
    jQuery(a.infoDiv).remove();
...
annotations.prototype.annotationWasDragged = function(a, event, ui) {
  ...
  jQuery(this).triggerHandler('annotationMoved', {
    annotation: a,
    oldYFrac: oldYFrac,
    newYFrac: a.yFrac
  });
  jQuery(this).triggerHandler('annotationsChanged', {});
};
...
annotations.prototype.makeAnnotationEditable = function(a) {
  ...
  a.editable = true;
  var editableTemplateDiv = jQuery('#annotation-editable-template').get(0);
  a.infoDiv.innerHTML = this.getTemplateHTML(editableTemplateDiv, a);
  jQuery(a.infoDiv).toggleClass('editable', !!a.editable);
  jQuery(this).triggerHandler('beganEditAnnotation', a);
};
...
annotations.prototype.createAnnotation = function(a) {
  ...
  var jQuerylineDiv = jQuery('<div/>').css({
    'width': '1px',
    'left': '3px',
    'background': 'black',
    'height': '100%',
  ...
  var jQueryinfoDiv = jQuery('#annotation-template').clone().removeAttr('id').css({
      'position': 'absolute',
      'border-color': color,
      'z-index': 10
    })
  ...
  jQuery.extend(a, {
    lineDiv: jQuerylineDiv.get(0),
    infoDiv: jQueryinfoDiv.get(0)
  });

  var that = this;

  jQueryinfoDiv.draggable({
  ...
  jQueryinfoDiv.on('click', '.annotation-kill-button', function() {
    that.removeAnnotation(a);
    jQuery(that).triggerHandler('annotationDeleted', a);
    jQuery(that).triggerHandler('annotationsChanged', {});
  });

  jQueryinfoDiv.on('dblclick', function() {
    that.makeAnnotationEditable(a);
  });
  jQueryinfoDiv.on('click', '.annotation-update', function() {
    self.extractUpdatedProperties_(jQueryinfoDiv.get(0), a);
    a.editable = false;
    self.updateAnnotationInfo();
    jQuery(that).triggerHandler('annotationEdited', a);
    jQuery(that).triggerHandler('annotationsChanged', {});
  });
  jQueryinfoDiv.on('click', '.annotation-cancel', function() {
    a.editable = false;
    self.updateAnnotationInfo();
    jQuery(that).triggerHandler('cancelEditAnnotation', a);
  });
...
annotations.prototype.moveAnnotationToTop = function(a) {
  ...
  jQuery(a.infoDiv).appendTo(div);
  jQuery(a.lineDiv).appendTo(div);
...
annotations.prototype.updateAnnotationDivPositions = function() {
  ...
  var that = this;
  jQuery.each(this.annotations_, function(idx, a) {
    var row_col = that.findPointIndex_(a.series, a.xval);
    if (row_col == null) {
      jQuery([a.lineDiv, a.infoDiv]).hide();
      return;
    } else {
      // TODO(danvk): only do this if they're invisible?
      jQuery([a.lineDiv, a.infoDiv]).show();
    }
    ...
    jQuery(a.lineDiv).css({
      'left': x + 'px',
      'top': Math.min(y, pointY) + 'px',
      'height': lineHeight + 'px'
    });
    jQuery(a.infoDiv).css({
      'left': x + 'px',
    });
    if (!a.isDragging) {
      ...
      jQuery([a.infoDiv, a.lineDiv]).toggle(visible);
    }
  });
};
...
annotations.prototype.updateAnnotationInfo = function() {
  ...
  var that = this;
  var templateDiv = jQuery('#annotation-template').get(0);
  jQuery.each(this.annotations_, function(idx, a) {
...
annotations.prototype.createPublicAnnotation_ = function(a, opt_props) {
  var displayAnnotation = jQuery.extend({}, a, opt_props);
...
annotations.prototype.extractUpdatedProperties_ = function(div, a) {
  jQuery(div).find('[dg-ann-field]').each(function(idx, el) {
    var k = jQuery(el).attr('dg-ann-field');
    var v = jQuery(el).val();
    a[k] = v;
  });
};
...
annotations.prototype.attachAnnotationsToChart_ = function() {
  ...
  jQuery.each(this.annotations_, function(idx, a) {
    ...
    jQuery([a.lineDiv, a.infoDiv]).appendTo(div);
  });
};
...
annotations.prototype.removeAnnotation = function(a) {
  ...
  if (idx >= 0) {
    this.annotations_.splice(idx, 1);
    jQuery([a.lineDiv, a.infoDiv]).remove();
  } else {
...
annotations.prototype.pointClick = function(e) {
  ...
  var a = jQuery.extend({}, this.defaultAnnotationProperties_, {
    series: e.point.name,
    xval: e.point.xval
  });
  ...
  jQuery(this).triggerHandler('annotationCreated', a);
  jQuery(this).triggerHandler('annotationsChanged', {});
...
annotations.prototype.set = function(annotations) {
  ...
  for (var i = 0; i < annotations.length; i++) {
    ...
    if (this.annotations_.length > i) {
      ...
      this.annotations_[i] = jQuery.extend({
