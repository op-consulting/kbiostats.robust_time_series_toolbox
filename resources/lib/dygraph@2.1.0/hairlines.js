...
hairlines.prototype.detachLabels = function() {
  for (var i = 0; i < this.hairlines_.length; i++) {
    ...
    jQuery(h.lineDiv).remove();
    jQuery(h.infoDiv).remove();
...
hairlines.prototype.hairlineWasDragged = function(h, event, ui) {
  ...
  jQuery(this).triggerHandler('hairlineMoved', {
    oldXVal: oldXVal,
    newXVal: h.xval
  });
  jQuery(this).triggerHandler('hairlinesChanged', {});
};
...
hairlines.prototype.createHairline = function(props) {
  ...
  var jQuerylineContainerDiv = jQuery('<div/>').css({
      'width': '6px',
      'margin-left': '-3px',
      'position': 'absolute',
      'z-index': '10'
  ...
  jQuerylineDiv.appendTo(jQuerylineContainerDiv);

  var jQueryinfoDiv = jQuery('#hairline-template').clone().removeAttr('id').css({
      'position': 'absolute'
    })
    .show();

  // Surely there's a more jQuery-ish way to do this!
  jQuery([jQueryinfoDiv.get(0), jQuerylineContainerDiv.get(0)])
    .draggable({
      'axis': 'x',
  ...
  h = jQuery.extend({
    interpolated: true,
    selected: false,
    lineDiv: jQuerylineContainerDiv.get(0),
    infoDiv: jQueryinfoDiv.get(0)
  }, props);

  var that = this;
  jQueryinfoDiv.on('click', '.hairline-kill-button', function(e) {
    that.removeHairline(h);
    jQuery(that).triggerHandler('hairlineDeleted', {
      xval: h.xval
    });
    jQuery(that).triggerHandler('hairlinesChanged', {});
...
hairlines.prototype.moveHairlineToTop = function(h) {
  ...
  jQuery(h.infoDiv).appendTo(div);
  jQuery(h.lineDiv).appendTo(div);
...
hairlines.prototype.updateHairlineStyles = function() {
  jQuery.each(this.hairlines_, function(idx, h) {
    jQuery([h.infoDiv, h.lineDiv]).toggleClass('selected', h.selected);
  });
};
...
hairlines.prototype.updateHairlineInfo = function() {
  ...
  jQuery.each(this.hairlines_, function(idx, h) {
    ...
    if (that.divFiller_) {
      ...
      jQuery('.hairline-legend', h.infoDiv).html(html);
    }
  });
};
...
hairlines.prototype.attachHairlinesToChart_ = function() {
  var div = this.dygraph_.graphDiv;
  jQuery.each(this.hairlines_, function(idx, h) {
    jQuery([h.lineDiv, h.infoDiv]).appendTo(div);
  });
};
...
hairlines.prototype.removeHairline = function(h) {
  ...
  if (idx >= 0) {
    this.hairlines_.splice(idx, 1);
    jQuery([h.lineDiv, h.infoDiv]).remove();
  } else {
...
hairlines.prototype.click = function(e) {
  ...
  this.addTimer_ = setTimeout(function() {
    ...
    jQuery(that).triggerHandler('hairlineCreated', {
      xval: xval
    });
    jQuery(that).triggerHandler('hairlinesChanged', {});
  }, CLICK_DELAY_MS);
